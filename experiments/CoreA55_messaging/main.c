...
int app (struct rpmsg_device *rdev, void *priv, unsigned long svcno)
{
    ...
    for (i = 0, size = pi.min; i < (int)pi.num-1; i++, size++) {
        i_payload->num = i;
        i_payload->size = size;
        ...
        usleep(1000); //usleep(100000); //usleep(10000);
    }

    int cnt = 20000; //10;
    LPRINTF("Continuing many times without inbetween printing...\n");
    while (cnt--) { //continue the test endlessly
        i_payload->num = i;
        i_payload->size = size;
