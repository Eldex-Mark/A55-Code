...
void _GUI_clearSelectorOptions (lv_obj_t* selector_widget, char* options ) {
    if ( lv_obj_check_type( selector_widget, &lv_roller_class ) )  options[0] = '\0';
...
void _GUI_createSelectorOptions (lv_obj_t* selector_widget, char* options) {
    if ( lv_obj_is_valid( selector_widget ) ) {
        if ( lv_obj_check_type( selector_widget, &lv_roller_class ) )  lv_roller_set_options( selector_widget, options, LV_ROLLER_MODE_INFINITE );
