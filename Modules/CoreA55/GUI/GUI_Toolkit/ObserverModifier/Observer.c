...
static inline void _GUI_refreshObserverWidget (_GUI_Observer* observer, _GUI_ValueContainer value) { //(lv_obj_t* widget, _GUI_SubjectVariableTypes type, _GUI_ValueContainer value, _GUI_valueConverterFunction convert, const char* format, _GUI_DisplayedTypes displayed_type) {
    static _GUI_SubjectVariableTypes TypeCategory; TypeCategory = _GUI_getTypeCategory( observer->VariableType );
    ...
    //if ( lv_obj_is_valid( widget ) ) {
        if (TypeCategory==_GUI_SUBJECT_TYPE_CATEGORY__INT/* || TypeCategory==_GUI_SUBJECT_TYPE_BOOL*/) {
            if (observer->DisplayedType == _GUI_DISPLAYED_TYPE__SHOW_HIDE) _GUI_conditionalShowWidget( *observer->WidgetPointer_pointer, value.Int );
            else _GUI_setWidgetFormattedText( *observer->WidgetPointer_pointer, observer->DisplayedType, _GUI_getFormatString( observer->FormatString_pointer,"%d" ), value.Int ); //lv_label_set_text_fmt is not enough
        }
        else if (TypeCategory==_GUI_SUBJECT_TYPE_CATEGORY__FLOAT) _GUI_setWidgetFormattedText( *observer->WidgetPointer_pointer, observer->DisplayedType, _GUI_getFormatString( observer->FormatString_pointer, "%f" ), value.Float );
        else if (TypeCategory==_GUI_SUBJECT_TYPE_CATEGORY__STRING)  _GUI_setWidgetFormattedText( *observer->WidgetPointer_pointer, observer->DisplayedType, _GUI_getFormatString( observer->FormatString_pointer, "%s" ), value.Pointer );
...
void _GUI_initScreenObservers (_GUI_Observer* observer_array) {
    ...
    for (int i=0; (VariableType = observer_array[i].VariableType) != _GUI_SUBJECT_TYPE__END; ++i) {
        _GUI_checkVariableChange( VariableType, observer_array[i].VariablePointer, &observer_array[i].PreviousValue, true );
    }
}
