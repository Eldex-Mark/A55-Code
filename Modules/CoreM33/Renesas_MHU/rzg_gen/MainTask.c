...
static void MainTask_func(void *pvParameters);
...
void MainTask_create(void)
{
    ...
    BaseType_t MainTask_create_err = xTaskCreate (
                    ...
                    if (NULL == MainTask)
                    {
                        rtos_startup_err_callback(MainTask, 0);
                    }
                    #else
    if (pdPASS != MainTask_create_err)
    {
        rtos_startup_err_callback (MainTask, 0);
    }
#endif
